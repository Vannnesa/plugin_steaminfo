...
async def broadcast_steam_info(
    ...
    # 这里仍然保留 UniMessage.send 的广播逻辑（若你也想改广播发送为 OneBot API，我们可以进一步改造）
    ...
    try:
        ...
        try:
            await uni_msg.send(Target(parent_id, parent_id, True, False, "", bot.adapter.get_name()))
        except Exception as e2:
            logger.exception(f"发送广播消息失败: {e2}")
...
async def info_handle(
    ...
    try:
        result = await draw_player_status(
        ...
        logger.exception(f"draw_player_status 调用异常: {e}")
        await info.finish("生成 Steam 信息时发生错误（绘图失败）")

    async def _read_path_bytes(path_like: Union[str, Path]) -> bytes:
        try:
            with open(str(path_like), "rb") as f:
                return f.read()
        except Exception as e:
            logger.error(f"无法读取文件 {path_like}: {e}")
            raise

    async def build_onebot_message_from_result(result) -> OneBotMessage:
        """
        将 draw_player_status 的返回 result 转换为 OneBotMessage (MessageSegment 列表)。
        支持:
          - {"type":"image","path"/"raw"/"url"}
          - {"type":"video","path"/"raw"/"url"}
          - bytes -> 视作图片
          - str (本地 path) -> 视作图片或视频，依据后缀
        """
        segments = []
        ...
        if isinstance(result, dict):
            ...
            if typ == "image":
                if "path" in result and result["path"]:
                    p = str(result["path"])
                    segments.append(MessageSegment.image(f"file://{p}"))
                    return OneBotMessage(segments)
                if "raw" in result and result["raw"] is not None:
                    path = write_tmp(bytes(result["raw"]), ".jpg")
                    segments.append(MessageSegment.image(f"file://{path}"))
                    return OneBotMessage(segments)
                if "url" in result and result["url"]:
                    segments.append(MessageSegment.image(result["url"]))
                    return OneBotMessage(segments)
                raise ValueError("image 类型缺少 path/raw/url")
            elif typ == "video":
                # 优先 path -> url -> raw
                if "path" in result and result["path"]:
                    p = str(result["path"])
                    segments.append(MessageSegment.video(f"file://{p}"))
                    return OneBotMessage(segments)
                if "url" in result and result["url"]:
                    segments.append(MessageSegment.video(result["url"]))
                    return OneBotMessage(segments)
                if "raw" in result and result["raw"] is not None:
                    path = write_tmp(bytes(result["raw"]), ".mp4")
                    segments.append(MessageSegment.video(f"file://{path}"))
                    return OneBotMessage(segments)
                raise ValueError("video 类型缺少 path/raw/url")
            elif typ == "error":
                raise RuntimeError(result.get("message", "draw 返回错误"))
            else:
                raise ValueError(f"未知 draw 返回类型: {typ}")

        # bytes -> image
        if isinstance(result, (bytes, bytearray)):
            path = write_tmp(bytes(result), ".jpg")
            segments.append(MessageSegment.image(f"file://{path}"))
            return OneBotMessage(segments)
        ...
        if isinstance(result, str):
            ...
            if p.exists():
                ...
                if suffix in [".mp4", ".mkv", ".webm", ".mov"]:
                    segments.append(MessageSegment.video(f"file://{str(p)}"))
                else:
                    segments.append(MessageSegment.image(f"file://{str(p)}"))
                return OneBotMessage(segments)
            # 不是本地文件 -> 视作 url
            if result.startswith("http://") or result.startswith("https://"):
                # 根据后缀判断
                lower = result.lower()
                if any(lower.endswith(ext) for ext in [".mp4", ".mkv", ".webm", ".mov"]):
                    segments.append(MessageSegment.video(result))
                else:
                    segments.append(MessageSegment.image(result))
                return OneBotMessage(segments)

        raise TypeError("无法将 draw 返回值转换为 OneBotMessage")

    # 构建并发送 OneBot 消息（只做一次 finish）
    try:
        onebot_msg = await build_onebot_message_from_result(result)
    except Exception as e:
        logger.exception(f"将 draw 返回值转换为 OneBotMessage 失败: {e}")
        await info.finish("生成 Steam 信息时发生错误（发送失败）")
        return

    # 成功构建后直接 finish（OneBotMessage 支持直接 finish）
    try:
        await info.finish(onebot_msg)
    except Exception as e:
        # 若 finish 抛出 FinishedException（通常是正常结束），只记录异常并不二次 finish
        logger.exception(f"发送 OneBotMessage 时发生异常: {e}")
        # 尝试回退到导出 UniMessage（仅作兜底，不强制再次 finish）
        try:
            uni_fallback = UniMessage(Image(raw=image_to_bytes(PILImage.new("RGB", (1, 1)))))
            # 不再调用 finish，以免二次 FinishedException；记录说明
            logger.warning("发送失败，已进入兜底逻辑（未发送）")
        except Exception:
            pass
...
async def check_handle(
    ...
    # 这里仍使用 UniMessage 发送检查结果以保持原行为（你也可以改为 OneBotMessage 同理）
